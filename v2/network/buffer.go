...
const (
	...
	UnregisterCommandApplication OutboundMessageTypes = 9
...
func MarshalDisconnectReq(buffer *bytes.Buffer, connectionId int32) (ok bool) {
	ok = writeByteBuffer(buffer, UnregisterCommandApplication)
...
func writeByteBuffer(buffer *bytes.Buffer, b byte) bool {
	...
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error in writeByteBuffer: %v", err)
		return false
	}
	return true
}
...
func writeBuffer(buffer *bytes.Buffer, data interface{}) bool {
	...
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error in writeBuffer: %v", err)
		return false
	}
	return true
}
...
func readBuffer(buffer *bytes.Buffer, data interface{}) bool {
	...
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error in readBuffer: %v:%+v", err, data)
		return false
	}
	return true
}
...
func writeString(buffer *bytes.Buffer, s string) bool {
	...
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error in writeString: %v", err)
		return false
	}
	buffer.Write([]byte(s))
	return true
...
func readString(buffer *bytes.Buffer, s *string) bool {
	...
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error in readString: %v", err)
		return false
	}
	stringBuffer := make([]byte, length)
	err = binary.Read(buffer, binary.LittleEndian, &stringBuffer)
	if err != nil {
		log.Error().Msgf("ACCBroadCastAPI: Error while reading in readStr: %v", err)
		return false
	}
